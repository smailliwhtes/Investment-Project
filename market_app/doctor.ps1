...
Write-Host "[stage] preflight stooq"
try {
  $r = Invoke-WebRequest "https://stooq.pl/q/d/l/?s=aapl.us&i=d" -UseBasicParsing -TimeoutSec 20
  $first = ($r.Content -split "`n")[0].Trim()
  if ($first -notlike "Date,Open,High,Low,Close,Volume*") {
    Write-Host "[stooq] blocked or non-CSV response: $first"
    exit 9
  }
} catch {
  Write-Host "[stooq] preflight failed: $(param(
  [string]$Config = ".\config.json"
)
...
Write-Host "  .\outputs\report.md".Exception.Message)"
  exit 9
}
Write-Host "[stage] running pipeline"
& $VenvPy ".\scripts\runner.py" --config $Config
