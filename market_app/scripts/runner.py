...
DAILY_LIMIT_TEXTS = [
    ...
    "Przekroczony dzienny limit wywołań"
]
DAILY_LIMIT_TEXT = DAILY_LIMIT_TEXTS[0]
...
def normalize_symbol(sym: str) -> str:
    sym = sym.strip().upper()
    # Skip symbols that are usually preferred/rights formats for which Stooq coverage is unreliable
    if "$" in sym:
        return ""
...
def stooq_csv_url(stooq_symbol: str) -> str:
    return f"https://stooq.pl/q/d/l/?s={stooq_symbol}&i=d"
...
def _log_stooq(msg: str) -> None:
    try:
        ensure_dir("outputs")
        with open(os.path.join("outputs","stooq_failures.log"), "a", encoding="utf-8") as f:
            f.write(msg + "\n")
    except Exception:
        pass

def download_stooq_daily(stooq_symbol: str, out_path: str, sleep_ms: int = 0) -> bool:
    url = stooq_csv_url(stooq_symbol)
    try:
        r = requests.get(url, timeout=30, headers={"User-Agent":"Mozilla/5.0"})
        if r.status_code != 200:
            _log_stooq(f"{stooq_symbol}\tHTTP_{r.status_code}\t{url}")
            return False
        txt = r.text or ""
        if any(t in txt for t in DAILY_LIMIT_TEXTS):
            raise StooqDailyLimit(DAILY_LIMIT_TEXT)

        if "Date,Open,High,Low,Close,Volume" not in txt:
            head = txt.splitlines()[0] if txt else "<empty>"
            _log_stooq(f"{stooq_symbol}\tNON_CSV\t{head[:140]}")
            return False
        ...
        return True
    except StooqDailyLimit:
        raise
    except Exception as e:
        _log_stooq(f"{stooq_symbol}\tEXC\t{type(e).__name__}:{e}")
        return False
...
def write_report(cfg: Dict, eligible: pd.DataFrame, scored: pd.DataFrame, universe_count: int, processed_count: int) -> None:
    ...
    if len(top) == 0:
        ...
        for _, r in top.iterrows():
            lines.append(f"- {r['symbol']}: {r['monitor_priority']:.2f} | px={r['last_close']:.4g} | adv20$={r['adv20_dollar']:.3g} | flags={r.get('risk_flags','')}")
    lines.append("")
...
def main():
    ...
    # Cursor rollback support if rate-limited mid-run
    state_after = read_json(paths["state_file"], default={"cursor": 0})
    cursor_after_select = int(state_after.get("cursor", 0))
    start_cursor = (cursor_after_select - len(batch)) % universe_count if universe_count else 0

    refresh_h = float(cfg["data"]["stooq_refresh_hours"])
    sleep_ms = int(cfg["data"].get("stooq_sleep_ms", 0))
    dl_cap = int(cfg["data"].get("max_stooq_downloads_per_run", 999999))
    dl_used = 0
    ...
    for i, sym in enumerate(batch, start=1):
        ...
        need_download = not file_is_fresh(csv_path, refresh_h)
        if need_download:
            if dl_used >= dl_cap:
                statuses.append({"symbol": sym, "status": "DOWNLOAD_CAP_REACHED"})
                break
            try:
                ok = download_stooq_daily(stooq_sym, csv_path, sleep_ms=sleep_ms)
                dl_used += 1
                if not ok:
                    statuses.append({"symbol": sym, "status": "DOWNLOAD_FAIL"})
                    attempted_for_cursor += 1
                    continue
            except StooqDailyLimit:
                # do NOT advance cursor past this symbol
                rate_limited = True
                statuses.append({"symbol": sym, "status": "DAILY_LIMIT_STOP"})
                break

        df = load_stooq_daily(csv_path)
        if df is None or len(df) < 5:
            statuses.append({"symbol": sym, "status": "PARSE_FAIL"})
            attempted_for_cursor += 1
            continue

        attempted_for_cursor += 1
        statuses.append({"symbol": sym, "status": "OK" if need_download else "CACHE_OK"})
    ...
    # Roll back cursor if we stopped due to daily limit so we don't skip the blocked symbol
    if rate_limited and universe_count:
        new_cursor = (start_cursor + attempted_for_cursor) % universe_count
        write_json(paths["state_file"], {"cursor": new_cursor, "updated_utc": datetime.now(timezone.utc).isoformat()})
        _p(f"[stooq] DAILY_LIMIT hit. Cursor rolled back to resume safely next run (cursor={new_cursor}).")

    # Always write status log for diagnosis
    pd.DataFrame(statuses).to_csv(os.path.join("outputs","ingest_status_last_run.csv"), index=False)

    scored_df = pd.DataFrame(rows_scored)
    feat_df   = pd.DataFrame(rows_features)

    if len(scored_df) == 0:
        _p("No symbols processed successfully in this run (data/provider constraints).")
        return 2 if not rate_limited else 3
