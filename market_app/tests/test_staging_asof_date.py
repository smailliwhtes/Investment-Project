...
def test_stage_pipeline_filters_anchor_before_lookback(tmp_path: Path) -> None:
    dates = pd.date_range("2019-01-01", "2026-01-27", freq="B")
    close = np.linspace(100.0, 200.0, len(dates))
    df = pd.DataFrame(
        {
            "Date": dates,
    ...
    _, _, stage3_df, _ = stage_pipeline(
        ...
        run_meta,
        logger=type("obj", (), {"info": lambda *args, **kwargs: None})(),
        now_utc="2023-02-24T21:00:00Z",
    )

    assert stage3_df.loc[0, "as_of_date"] == "2023-02-24"
    assert stage3_df.loc[0, "history_days"] == 600
    assert stage3_df.loc[0, "history_days_micro"] == 7
    assert stage3_df.loc[0, "history_days_short"] == 60
    assert stage3_df.loc[0, "history_days_deep"] == 600
