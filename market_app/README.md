...
To harden determinism, anchor the run to a fixed UTC day (and optionally a full timestamp):

**Windows (PowerShell)**
```powershell
python -m market_app.cli run --config .\config.yaml --offline --as-of-date 2025-01-31 --now-utc 2025-01-31T00:00:00Z
```
...
- doctor + determinism check complete with required artifacts
- determinism diffs (if any) are written to `outputs/runs/determinism_check/<run_id>`

## Determinism check (single source of truth)

The CLI can run two consecutive deterministic passes and compare stable artifacts:

```powershell
python -m market_app.cli determinism-check --config .\config.yaml --offline --as-of-date 2025-01-31 --run-id det_check
```

If diffs exist, the tool writes:
- `diff_summary.json` with per-file diff metadata
- `diff_report.md` with a human-readable summary
- `diff_examples_<file>.csv` samples for CSV mismatches

To allowlist a *specific* column (use sparingly), add to config:
