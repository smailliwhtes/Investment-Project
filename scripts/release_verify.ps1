...
function Invoke-LoggedCommand {
    ...
    try {
        ...
        return @{ ExitCode = $LASTEXITCODE; Log = $logPath }
    }
    finally {
        Pop-Location
    }
...
try {
    ...
    if ($SkipGuiSmoke -or -not $IsWindows) {
        ...
        $proc = Start-Process dotnet -ArgumentList @('run','--project',$guiProj.FullName,'--','--smoke') -PassThru -NoNewWindow -RedirectStandardOutput $guiLog -RedirectStandardError $guiLog
