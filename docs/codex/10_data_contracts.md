...
GDELT events files that are tab-delimited even if the extension is `.csv`.

Outputs are partitioned by day:
...
Important:
- Strict no-leakage joining: when creating features for date D, only use GDELT rows with day <= D for
  lagged/rolling features.

## E) Corpus Doctor (raw vs stable vs normalized)
Definitions:
...
Normalized cache layout (events):
...
- inputs (raw_files fingerprints)
- config (date_col, write_format, aggregation rules)
- classification_summary (files per type)
- content_hash
...
## F) Joined market + GDELT daily features
...
- gdelt_features... (lagged/rolling variants only by default)

Lag/rolling rules:
- GDELT lags: per feature, t-1/t-3/t-7 (configurable)
- Optional rolling mean/sum for count-like features (default window 7)
- **No leakage**: same-day exogenous columns are excluded unless explicitly enabled
...
- config (lags, rolling settings, include_raw_gdelt)
